...
export class PostsService {
  ...
  constructor() {
    let post = new Post();
    post.id = '1';
    post.publishedAt = new Date(2019, 10, 20, 20, 39, 24);
    post.tag = "Javascript";
    post.title = `Destructuring Assignment in JavaScript`;
    post.intro = `In this post I am going to explain one of the more exciting new features of javascript. Although it is not as disruptive for javascript landscape as some other new features such as <code>async/await</code> but it is pretty handy and instrumental to use. that feature is Object Destructuring.`
    post.content = `<p>Hi there!</p><p>As I continue my journey into the world of JavaScript (JS), one technique that was new to me that I’ve found using a lot is a technique called destructuring assignment.</p><p>Let’s walk through some examples.</p><p>Typically in JS, you might assign a value to a variable like this.<br></p><div class="highlight"><preclass="highlight plaintext">
    <code>let myString = 'my string';
    </code></pre></div>
    <p>But what if the value is not literal, and coming from somewhere else? What if its an object and you want to pluck just a few things from it. You might write something like this.<br></p><div class="highlight"><pre class="highlight plaintext"><code>let myObject = {myString: 'foo', myInt: 42, myBool: true};
